...
This document describes all configuration options and settings available in the Bisly Home Assistant Websockets Integration custom integration.
...
```yaml
service: hacs_bisly.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: hacs_bisly.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Bisly Home Assistant Websockets Integration Alert
...
- [GitHub Issues](https://github.com/klarstil/hacs-bisly/issues) - Report problems
