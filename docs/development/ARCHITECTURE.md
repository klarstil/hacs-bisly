...
This document describes the technical architecture of the Bisly Home Assistant Websockets Integration custom component for Home Assistant.

## Directory Structure

```text
custom_components/hacs_bisly/
...
│   ├── __init__.py          # Exports BislyDataUpdateCoordinator
...
│   ├── __init__.py          # Exports BislyEntity
...
- `base.py` - Main coordinator class (`BislyDataUpdateCoordinator`)
...
**Key class:** `BislyDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `BislyConfigFlowHandler` (main flow)
- `BislyOptionsFlow` (options)
...
**Key class:** `BislyEntity` (in `entity/base.py`)
...
2. `BislyEntity` for common functionality
...
1. Create directory: `custom_components/hacs_bisly/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `BislyEntity`
