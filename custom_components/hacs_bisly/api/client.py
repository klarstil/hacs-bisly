...
class BislyApiClientCommunicationError(
    BislyApiClientError,
):
    """Exception to indicate a communication error with the API."""


class BislyApiClientAuthenticationError(
    BislyApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        BislyApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise BislyApiClientAuthenticationError(
            msg,
        )
...
class BislyApiClient:
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise BislyApiClientCommunicationError(
                msg,
            ) from exception
