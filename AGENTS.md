...
- **Repository:** klarstil/hacs-bisly

**Key directories:**

- `custom_components/hacs_bisly/` - Main integration code
...
- Integration logs: `custom_components.hacs_bisly: debug` in `config/configuration.yaml`
...
- Use the domain `hacs_bisly` for all DOMAIN references
- Prefix all integration-specific classes with `Bisly`
- Use "Bisly Home Assistant Websockets Integration" as the display title
...
- Inherit from platform base + `BislyEntity`
...
- `tests/` mirrors `custom_components/hacs_bisly/` structure
